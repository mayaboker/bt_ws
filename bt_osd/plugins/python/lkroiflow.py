...
class LkRoiFlow(GstBase.BaseTransform):
    __gstmetadata__ = (
        "LK ROI Optical Flow",
        "Filter/Effect/Video",
        "Lucas-Kanade optical flow inside selected ROI",
        "Amir",
    )
    ...
    def do_transform_ip(self, buf: Gst.Buffer):
        ...
        try:
            ...
            if roi:
                ...
                self._post_roi_metadata(buf, roi, dx, dy, len(good_new))
    ...
    def _post_roi_metadata(self, buf, roi, dx, dy, point_count):
        x1, y1, x2, y2 = roi
        structure = Gst.Structure.new_empty("lkroiflow-roi")
        structure.set_value("x1", int(x1))
        structure.set_value("y1", int(y1))
        structure.set_value("x2", int(x2))
        structure.set_value("y2", int(y2))
        structure.set_value("dx", float(dx))
        structure.set_value("dy", float(dy))
        structure.set_value("points", int(point_count))
        structure.set_value("pts", GLib.Variant("t", int(buf.pts)))
        self.post_message(Gst.Message.new_element(self, structure))


GObject.type_register(LkRoiFlow)
__gstelementfactory__ = ("lkroiflow", Gst.Rank.NONE, LkRoiFlow)
