...
`GstAnalyticsODMtd` to the original RGB video buffer. The standard
`objectdetectionoverlay` element draws the results. Preprocessing, letterbox
mapping, decoding, and per-class NMS use standard C++ and do not require OpenCV.
...
gst-launch-1.0 -e \
  ...
  objectdetectionoverlay draw-labels=true ! videoconvert ! autovideosink sync=false
```
