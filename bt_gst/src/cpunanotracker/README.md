...
Run tracking from the Gazebo camera. Replace the ROI with your target's
`x,y,width,height` in the first frame:
...
The equivalent checked script is `scripts/run_cpunanotracker_gz.sh`. Override
`GZ_IMAGE_TOPIC`, `NANOTRACK_ROI`, or `NANOTRACK_MODELS_DIR` as needed.
