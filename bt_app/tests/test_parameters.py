...
def test_canonical_parameter_registry_has_49_mavlink_names():
    service = make_service()

    assert len(ALL_PARAMETER_KEYS) == 49
...
def test_list_read_and_int_set_use_canonical_names():
    ...
    assert len(responses) == 49
    assert responses[0].message.param_id == "FS_HOLD_TIME"
    assert responses[-1].delay_s == pytest.approx(0.96)
