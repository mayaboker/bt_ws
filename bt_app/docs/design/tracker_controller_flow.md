...
The public lifecycle is acquisition, tracking, commit, and stop. `TrackerPhase.TERMINAL` remains in the enum for API compatibility but is not entered by this implementation.
...
stateDiagram-v2
    ...
    Ready --> Tracking: start_tracking + fresh vario
...
`start_tracking(...)` rejects entry unless acquisition is ready and vario is fresh. It initializes:

- pitch to `TTC_PIT_INIT`;
...
| Yaw | `TRK_YAW_KP=30`, `TRK_YAW_MAX=20`, `TRK_DEADBAND=0.03` |
...
- pitch, yaw, throttle, commit state, exit state;
...
- no IMU attitude feedback in the pitch loop;
