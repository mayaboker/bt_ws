...
```text
yaw_rate = clamp(
    TRK_YAW_SIGN * TRK_YAW_KP * deadband(error_x, TRK_DEADBAND),
    -TRK_YAW_MAX,
    +TRK_YAW_MAX,
)
```
...
`HOV_BASELINE` and the `BF_YAW_CENTER`/`BF_YAW_RATE`/`BF_YAW_EXPO` Actual-rates
parameters remain the shared hover and yaw-mapping
