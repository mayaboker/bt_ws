#!/usr/bin/env python3
"""Soft manual takeoff and forward/back pitch maneuver scenario."""
...
PITCH_SOFT_FORWARD_RC = 1600
PITCH_SOFT_BACK_RC = 1400
PITCH_DURATION_S = 3.0
PITCH_STABILIZE_DURATION_S = 5.0
PITCH_CHANNEL = 1
...
def pitch_channels(pitch_rc: int) -> tuple[int, ...]:
    channels = list(ALT_HOLD_ARMED)
    channels[PITCH_CHANNEL] = pitch_rc
    return tuple(channels)


class TakeoffPitchScenario(MavlinkRcScenarioBase):
    """Reuse the roll scenario's PID climb/descent and test pitch softly."""
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self._phase(f"Soft pitch forward for {PITCH_DURATION_S} seconds")
            self._send_for(pitch_channels(PITCH_SOFT_FORWARD_RC), PITCH_DURATION_S)
            self._phase("Stabilizing pitch")
            self._send_for(
                ALT_HOLD_ARMED,
                PITCH_STABILIZE_DURATION_S,
            )
            self._phase(f"Soft pitch back for {PITCH_DURATION_S} seconds")
            self._send_for(pitch_channels(PITCH_SOFT_BACK_RC), PITCH_DURATION_S)
            self._phase("Centering pitch and stabilizing")
            self._send_for(
                ALT_HOLD_ARMED,
                PITCH_STABILIZE_DURATION_S,
            )
            ...
            if self.telemetry.state != STATE_ALT_HOLD:
                raise ScenarioError(
                    "Vehicle left ALT_HOLD during pitch maneuver; "
